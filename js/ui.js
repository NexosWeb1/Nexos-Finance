/* ============================================================
   ui.js — helpers de UI, formatação e toasts
   ============================================================ */

/* ---- Formatação monetária (valores em CENTAVOS) ---- */
export function fmtBRL(cents) {
  const v = (Number(cents) || 0) / 100;
  return v.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
}

export function fmtBRLshort(cents) {
  const v = (Number(cents) || 0) / 100;
  if (Math.abs(v) >= 1000) {
...
/* "R$ 1.234,56" ou "1.234,56" -> centavos (inteiro) */
...
export const mesesAbbr = MESES_ABBR;
export const diasSemana = DOW;

/* ISO yyyy-mm-dd a partir de Date (local, sem fuso) */
...
/* dd/mm/aaaa -> yyyy-mm-dd (retorna "" se inválida) */
export function brToISO(br) {
  const m = String(br).match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
  if (!m) return "";
  const [, d, mo, y] = m;
  const dt = new Date(Number(y), Number(mo) - 1, Number(d));
  if (dt.getDate() !== Number(d) || dt.getMonth() !== Number(mo) - 1) return "";
  return `${y}-${mo}-${d}`;
}

export function isoLongLabel(iso) {
  const d = fromISO(iso);
  return `${DOW[d.getDay()]}, ${d.getDate()} de ${MESES[d.getMonth()]} de ${d.getFullYear()}`;
}
...
export const icons = {
  entradas: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20M2 12h20"/></svg>',
  wallet: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 7V5a2 2 0 0 0-2-2H5a2 2 0 0 0 0 4h14a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5"/><path d="M18 12h.01"/></svg>',
  tasks: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-6"/><path d="m9 11 3 3L22 4"/></svg>',
  ...
  calWeek: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18M8 2v4M16 2v4"/></svg>',
  ...
  contract: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M9 13h6M9 17h4"/></svg>',
...
export function toast(message, type = "info", timeout = 3200) {
  ...
  const ic = type === "success" ? icons.check : type === "error" ? icons.alert : icons.alert;
  const node = el("div", { class: `toast ${type}` },
    el("span", { class: "ic", html: ic }),
    el("span", {}, message)
  );
